...
class FastWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        if text_dim is None:
            ...
            text_dim = int(self.text_encoder.model.dim)
    ...
    def from_wan22_pretrained(
        ...
        video_dit_config = {**WAN22_TI2V_5B_VIDEO_CONFIG, **(video_dit_config or {})}
        action_dit_config = {**FASTWAM_ACTION_CONFIG, **(action_dit_config or {})}
    ...
    def encode_prompt(self, prompt: Union[str, Sequence[str]]):
        if self.text_encoder is None or self.tokenizer is None:
            raise ValueError("Prompt encoding requires loaded text encoder/tokenizer. Set `load_text_encoder=true` or provide precomputed `context/context_mask`.")
        ids, mask = self.tokenizer(prompt, return_mask=True, add_special_tokens=True)
        ids = ids.to(self.device)
        mask = mask.to(self.device, dtype=torch.bool)
        prompt_emb = self.text_encoder.model(ids, mask)
    ...
    def _append_proprio_to_context(
        ...
        proprio_token = self.proprio_encoder(proprio.to(device=self.device, dtype=context.dtype).unsqueeze(1)).to(dtype=context.dtype)  # [B, 1, D]
    ...
    def _encode_video_latents(self, video_tensor, tiled=False, tile_size=(30, 52), tile_stride=(15, 26)):
        ...
        if tiled:
            raise ValueError("Wan2.2 training VAE does not support tiled encoding.")
        latents = self.vae.encode([video for video in video_tensor])
        return torch.stack(latents).to(device=self.device, dtype=self.torch_dtype)
    ...
    def _encode_input_image_latents_tensor(self, input_image: torch.Tensor, tiled=False, tile_size=(30, 52), tile_stride=(15, 26)):
        ...
        if tiled:
            raise ValueError("Wan2.2 training VAE does not support tiled encoding.")
        return torch.stack(self.vae.encode([image])).to(device=self.device, dtype=self.torch_dtype)

    def _decode_latents(self, latents, tiled=False, tile_size=(30, 52), tile_stride=(15, 26)):
        del tile_size, tile_stride
        if tiled:
            raise ValueError("Wan2.2 training VAE does not support tiled decoding.")
        video_tensor = torch.stack(self.vae.decode([latent for latent in latents]))
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        if action_horizon % (num_frames - 1) != 0:
            raise ValueError(f"`sample['action']` temporal dimension must be divisible by video transitions ({num_frames - 1}), got {action_horizon}")
    ...
    def _compute_video_loss_per_sample(
        ...
        if (image_is_pad.shape[1] - 1) % temporal_factor != 0:
            raise ValueError(f"Cannot align `image_is_pad` with video latent steps: num_frames={image_is_pad.shape[1]}, temporal_downsample_factor={temporal_factor}.")
        ...
        if video_is_pad.shape[1] != video_loss_token.shape[1]:
            raise ValueError(f"Video-loss mask shape mismatch: mask steps={video_is_pad.shape[1]}, loss steps={video_loss_token.shape[1]}.")
