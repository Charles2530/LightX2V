...
@MODEL_REGISTER("wan_fasterwam")
class WanFasterWAMModel:
    ...
    def load_components(self, transformer_only=False, reference_model=None):
        ...
        self.module = FasterWAM.from_wan22_pretrained(
            model_id=model_path,
            device=str(self.device),
            torch_dtype=self.running_dtype,
            tokenizer_max_len=int(self.model_config.get("tokenizer_max_len", 128)),
            load_text_encoder=load_text_encoder,
            redirect_common_files=bool(self.model_config.get("redirect_common_files", True)),
            ...
            condition_layers=self.model_config.get("condition_layers"),
            video_kv_fusion=self.model_config.get("video_kv_fusion"),
            video_kv_fusion_init=self.model_config.get("video_kv_fusion_init", "current_layer"),
        )

        checkpoint_path = self.model_config.get("checkpoint_path")
        if checkpoint_path:
            logger.info("[model] loading FasterWAM checkpoint {}", checkpoint_path)
            # FastWAM checkpoints contain dense ActionDiT projections, while
            # FasterWAM uses SparseActionDiT.  Load only shape-compatible
            # tensors so the shared video/MoT weights are retained without
            # making the original FasterWAM loader permissive globally.
            payload = torch.load(checkpoint_path, map_location="cpu")
            module = self.unwrap_module()
            state = payload.get("mot", payload.get("dit", payload))
            current = module.mot.state_dict()
            ...
            module.mot.load_state_dict(compatible, strict=False)
            if getattr(module, "proprio_encoder", None) is not None and "proprio_encoder" in payload:
                module.proprio_encoder.load_state_dict(payload["proprio_encoder"], strict=False)
            logger.info("[model] loaded {} compatible checkpoint tensors; skipped {} shape-incompatible tensors", len(compatible), skipped)
    ...
    def log_model_structure(self):
        module = self.unwrap_module()
        logger.info("[model] class=WanFasterWAMModel fastwam_class={}", module.__class__.__name__)
